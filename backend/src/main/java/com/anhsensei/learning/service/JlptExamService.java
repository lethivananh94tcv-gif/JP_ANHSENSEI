...
import com.anhsensei.learning.dto.ConfirmJlptImportRequest;
...
public class JlptExamService {
    ...
    @Transactional
    public JlptExam confirmImportExam(ConfirmJlptImportRequest req) {
        String examCode = req.getExamCode() != null && !req.getExamCode().isBlank()
                ? req.getExamCode().trim()
                : req.getLevelCode().toLowerCase() + "-" + UUID.randomUUID().toString().substring(0, 8);

        JlptExam exam = examRepository.findAll().stream()
                .filter(e -> e.getCode().equalsIgnoreCase(examCode))
                .findFirst()
                .orElseGet(() -> {
                    JlptExam newExam = new JlptExam(examCode, req.getLevelCode().toUpperCase(), req.getYearSession(), req.getYearSession());
                    return examRepository.save(newExam);
                });

        int nextVerNum = 1;
        List<JlptExamVersion> existingVersions = versionRepository.findByExamIdOrderByVersionNumberDesc(exam.getId());
        if (!existingVersions.isEmpty()) {
            nextVerNum = existingVersions.get(0).getVersionNumber() + 1;
        }

        JlptExamVersion version = new JlptExamVersion(
                exam.getId(),
                nextVerNum,
                req.getPdfUrl() != null ? req.getPdfUrl() : "/pdf/jlpt/" + req.getLevelCode().toLowerCase() + "/" + examCode + ".pdf",
                req.getAudioUrl() != null ? req.getAudioUrl() : "/audio/jlpt/" + req.getLevelCode().toLowerCase() + "/" + examCode + ".mp3",
                req.getDurationMinutes() != null ? req.getDurationMinutes() : 105
        );
        version.setStatus("PUBLISHED");
        version.setPublishedAt(Instant.now());
        version = versionRepository.save(version);
        ...
        if (req.getQuestions() != null) {
            for (ConfirmJlptImportRequest.ParsedQuestionItem item : req.getQuestions()) {
                JlptExamQuestion q = new JlptExamQuestion(
                        version.getId(),
                        item.getGlobalIndex(),
                        item.getLocalPdfNumber() != null ? item.getLocalPdfNumber() : item.getGlobalIndex(),
                        item.getSectionType(),
                        item.getSnippet() != null ? item.getSnippet() : "Câu hỏi " + item.getGlobalIndex(),
                        item.getCorrectOption() != null ? item.getCorrectOption() : 1,
                        "Phương án [" + (item.getCorrectOption() != null ? item.getCorrectOption() : 1) + "]",
                        item.getExplanation() != null ? item.getExplanation() : ""
                );
                if (item.getAudioScriptJa() != null) {
                    q.setAudioScript(item.getAudioScriptJa());
                }
