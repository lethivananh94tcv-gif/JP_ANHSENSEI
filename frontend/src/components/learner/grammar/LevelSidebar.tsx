...
export default function LevelSidebar({
  ...
  return (
    <div className="bg-[#FFFDF9] border border-[#EFE5DA] rounded-2xl p-3 sm:p-4.5 shadow-2xs space-y-2 sm:space-y-3.5">
      <h3 className="text-[11px] font-extrabold font-sans text-[#76685F] uppercase tracking-wider px-1">
        Chọn cấp độ
      </h3>

      <div className="flex xl:flex-col overflow-x-auto gap-2 xl:gap-2 pb-1 xl:pb-0 scrollbar-none">
        {levels.map((lvl) => {
          ...
          return (
            <button
              ...
              className={`shrink-0 xl:w-full p-2.5 sm:p-3 rounded-xl transition-all duration-200 cursor-pointer flex items-center gap-2.5 sm:gap-3 text-left border ${
                isActive
            ...
            >
              ...
              <div>
                ...
                <p className={`text-[10px] sm:text-[11px] font-medium font-sans mt-0.5 whitespace-nowrap ${isActive ? "text-white/90" : "text-[#76685F]"}`}>
                  {lvl.name}
                </p>
              </div>
            </button>
