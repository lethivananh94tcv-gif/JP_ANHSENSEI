...
export default function GrammarClozeTransformPractice({
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-3xl p-8 border border-amber-100 shadow-sm text-center py-16">
        <RefreshCw className="w-10 h-10 text-amber-500 animate-spin mx-auto mb-4" />
        <p className="text-stone-600 font-medium">Đang tải bài tập biến đổi thể & điền trợ từ...</p>
      </div>
    );
  }
  ...
  if (isFinished) {
    ...
    return (
      <div className="bg-white rounded-3xl p-8 border border-amber-100 shadow-sm text-center">
        ...
        <h3 className="text-2xl font-bold text-stone-800 mb-2">Hoàn Thành Bài Luyện Tập!</h3>
        <p className="text-stone-600 mb-6">
          Bạn đạt <span className="text-amber-600 font-bold">{score}/{questions.length}</span> câu chính xác ({percentage}%).
        </p>

        <div className="flex justify-center gap-4">
          <button
            onClick={handleRestart}
            className="flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white font-bold px-6 py-3 rounded-2xl shadow-lg shadow-amber-200 transition-all hover:scale-105"
          >
    ...
    <div className="bg-white rounded-3xl p-6 sm:p-8 border border-amber-100 shadow-sm">
      ...
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-stone-100">
        ...
        <div className="flex items-center gap-2 text-stone-400 text-sm">
          <Volume2 className="w-4 h-4 cursor-pointer hover:text-amber-600 transition-colors" />
        </div>
      </div>

      {/* Main Prompt Card */}
      <div className="bg-gradient-to-br from-amber-50/50 to-orange-50/30 rounded-2xl p-6 mb-8 border border-amber-100/60">
        <h4 className="text-xs font-bold text-amber-700 uppercase tracking-wider mb-2">
          {currentQ.title}
        </h4>
        <div className="text-2xl sm:text-3xl font-bold text-stone-800 tracking-wide mb-3 leading-relaxed">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        {currentQ.options.map((option, idx) => {
          let btnStyle = "bg-stone-50 hover:bg-amber-50/60 border-stone-200 text-stone-700";

          if (isAnswered) {
            if (option === currentQ.correctAnswer) {
              btnStyle = "bg-emerald-100 border-emerald-400 text-emerald-800 font-bold shadow-sm";
            ...
            btnStyle = "bg-amber-100 border-amber-400 text-amber-900 font-bold ring-2 ring-amber-400/30";
          }
          ...
          return (
            <button
              ...
              disabled={isAnswered}
              className={`p-4 rounded-2xl border text-left transition-all flex items-center justify-between ${btnStyle}`}
            >
      ...
      {isAnswered && (
        ...
        >
          ...
          <p className="text-sm mt-1 text-stone-700 leading-relaxed">
        ...
        {!isAnswered ? (
          <button
            ...
            className={`px-8 py-3.5 rounded-2xl font-bold transition-all shadow-md ${
              selectedOption
                ? "bg-amber-500 hover:bg-amber-600 text-white shadow-amber-200 hover:scale-105"
          ...
          <button
            ...
            className="flex items-center gap-2 px-8 py-3.5 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-2xl shadow-lg shadow-amber-200 transition-all hover:scale-105"
          >
